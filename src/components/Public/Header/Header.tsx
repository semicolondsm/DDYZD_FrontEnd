...
import { setState } from "@/src/context/context";
import Modal from "../Modals/Menu";
...
function Header({ color }: { color: string }) {
    ...
    useEffect(() => {
        ...
        localStorage.userCache &&
            setUserState(JSON.parse(localStorage.userCache));
    }, []);
    ...
    return (
        <div>
            <h.Header>
                ...
                <h.BottomHeader>
                    <ul>
                        <div onClick={ModalOn} style={{ cursor: "pointer" }}>
                            <ListIco></ListIco>
                        </div>
                        <h3 onClick={ModalOn} style={{ cursor: "pointer" }}>
                            전체 카테고리
                        </h3>
                        <li>
                            <Link href="/majorlist">
                        ...
                        <li onClick={() => alert("개발중")}>
                            동아리 물품 신청
                        </li>
                        <li
                            onClick={() =>
                                (window.location.href =
                                    "https://ddyzd-club.dsmkr.com/")
                            }
                        >
                            관리 페이지
                        </li>
                    ...
                    <ul>
                        {user_state ? (
                            <>
                                <li>
                                    <Link
                                        href={`/profile?id=${user_state.gcn}`}
                                    >
                                        <a>{user_state?.name}</a>
                                    </Link>
                                </li>
